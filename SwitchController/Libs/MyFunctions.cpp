...
{
	bool* buf = new bool[arr_len];
	for (uint8_t i = 0; i < arr_len; i++) buf[i] = strncmp(str, arr[i], len);
	uint8_t res = arraySearch(buf, true, arr_len);
	delete[] buf;
	return res;
} // strcmp_A
...
uint8_t strcmp_AP(const char* str, uint8_t len, char const* const* arr, uint8_t arr_len)
{
	bool* buf = new bool[arr_len];
	uint8_t i;
	for (i = 0; i < arr_len; i++) buf[i] = strncmp_P(str, pgm_read_PGM_P(arr[i]), len);
	uint8_t res = arraySearch(buf, true, arr_len);
	delete[] buf;
	return res;
} // strcmp_AP
